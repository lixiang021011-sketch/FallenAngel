...
- **Envisioned shape**: a **roguelike rhythm game** — in-run buffs and debuffs change the *information* the player gets (the judgement line disappears, timing windows scale, the chart goes invisible) while the feel of judgement itself stays fixed. In-run modifiers are not implemented yet (see architecture.md §9).
