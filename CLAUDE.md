...
8. **美术先过稿再进游戏**（2026-09-14 作者要求）：任何视觉组件的改动——背景、音符、轨道、判定线、HUD、特效——**必须先在 `art_tools/` 出效果图给作者过稿**，通过后才动 `Assets/Scripts` 与 `Assets/Art`。
   把程序化几何"换个颜色"不算完成；效果图阶段不要提前改游戏代码。调试工具（如 modifier 调试窗口）不在此列。
